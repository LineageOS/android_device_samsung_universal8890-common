...
{
    ALOGV("%s->%08lX->%08X", __FUNCTION__, (uintptr_t)device,
...
static int camera3_register_stream_buffers(const camera3_device *device, const camera3_stream_buffer_set_t *buffer_set) __unused
{
    ALOGV("%s->%08lX->%08X", __FUNCTION__, (uintptr_t)device,
        (uintptr_t)(((wrapper_camera3_device_t*)device)->vendor));
        ...
        hw_device_t **device)
{
    ...
    if (name != NULL) {
        ...
        ALOGV("%s: got vendor camera device 0x%08lX", __FUNCTION__, (uintptr_t)(camera3_device->vendor));
